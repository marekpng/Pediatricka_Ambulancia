<template>
  <div class="container py-5">
    ...
    <section v-if="Object.keys(groupedSchedules).length" class="mb-5">
      <h2>Select Year</h2>
      <div class="row">
        <div v-for="year in Object.keys(groupedSchedules)" :key="year" class="col-md-3 mb-3">
          <button class="btn btn-primary w-100" @click="selectYear(year)">
            {{ year }}
          </button>
        </div>
      </div>
    ...
    <section v-if="selectedYear" class="mb-5">
      ...
      <div class="row">
        <div v-for="month in Object.keys(groupedSchedules[selectedYear])" :key="month" class="col-md-3 mb-3">
          <button class="btn btn-primary w-100" @click="selectMonth(month)">
            {{ month }}
          </button>
        </div>
      </div>
    ...
    <section v-if="selectedMonth" class="mb-5">
      ...
      <div class="row">
        <div v-for="date in Object.keys(groupedSchedules[selectedYear][selectedMonth])" :key="date" class="col-md-3 mb-3">
          <button class="btn btn-primary w-100" @click="selectDate(date)">
            {{ date }}
          </button>
        </div>
      </div>
    ...
    <section v-if="selectedDate" class="mb-5">
      ...
      <ul class="list-group">
        <li v-for="timeslot in groupedSchedules[selectedYear][selectedMonth][selectedDate]" :key="timeslot.id" class="list-group-item d-flex justify-content-between align-items-center">
          <div>
            {{ timeslot.start_time }} - {{ timeslot.end_time }}
            <span v-if="timeslot.is_booked" class="badge bg-success ms-2">Booked</span>
            <span v-else class="badge bg-warning ms-2">Available</span>
          </div>
          <div>
            <button class="btn btn-warning btn-sm me-2" @click="editTimeslot(timeslot)" :disabled="timeslot.is_booked">
              Edit
            </button>
            <button class="btn btn-danger btn-sm" @click="deleteTimeslot(timeslot.id)" :disabled="timeslot.is_booked">
              Delete
            </button>
          </div>
        </li>
  ...
  methods: {
    async fetchSchedules() {
      ...
      try {
        ...
        // Convert Slovak date format (d.m.Y) to ISO (YYYY-MM-DD)
        this.schedules = response.data.timeslots.map((timeslot) => ({
          ...timeslot,
          date: this.convertSlovakToISO(timeslot.date),
        }));
    ...
    groupSchedules() {
      this.groupedSchedules = this.schedules.reduce((acc, timeslot) => {
        ...
        const date = this.convertISOToSlovak(timeslot.date); // Convert back to Slovak format for UI
     ...
     convertSlovakToISO(date) {
    const parts = date.split(".");
      if (parts.length === 3) {
        const [day, month, year] = parts.map(num => num.trim().padStart(2, "0"));
    ...
    editTimeslot(timeslot) {
      ...
      this.editingTimeslot.date = this.convertSlovakToISO(timeslot.date); // Convert to `YYYY-MM-DD`
    },

    async saveTimeslot() {
      if (this.setLoading) this.setLoading(true);
      try {
        const formattedDate = this.convertSlovakToISO(this.editingTimeslot.date); // Convert "3.2.2025" → "2025-02-03"
        ...
        await axios.put(
          ...
          {
            date: formattedDate, // Ensure it's in YYYY-MM-DD format
